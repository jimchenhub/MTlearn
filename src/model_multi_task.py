...
class HomoMultiTaskModel(object):
    ...
    def __init__(self, num_tasks, network_name, output_num, gpus, file_out, trade_off=1.0,
                 optim_param={"init_lr": 0.00003, "gamma": 0.3, "power": 0.75, "stepsize": 3000}):
        def select_func(x):
            if x > 0.1:
                return 1. / x
            else:
        ...
        self.device = torch.device("cuda:"+",".join(gpus))
        self.shared_layers = network_dict[self.network_name]()#.to(self.device)
        self.networks = [[nn.Linear(self.shared_layers.output_num(), self.output_num).to(self.device)]
                         for _ in range(self.num_tasks)]  # layers not shared but related
        ...
        self.shared_layers = nn.DataParallel(self.shared_layers, device_ids=gpu_ids)
    ...
    def optimize_model(self, input_list, label_list):
        ...
        output_list = [self.networks[i](feature_out.narrow(0, i*batch_size, batch_size)) for i in range(self.num_tasks)]
        ...
        if self.trade_off > 0 and self.iter_num % self.cov_update_freq == 0:
            ...
            temp_task_cov_var = tensor_op.UpdateCov(weights.data,
                                                    self.class_cov_var.data, self.feature_cov_var.data)
            temp_class_cov_var = tensor_op.UpdateCov(weights.data.permute(1, 0, 2).contiguous(),
                                                     self.task_cov_var.data, self.feature_cov_var.data)
            temp_feature_cov_var = tensor_op.UpdateCov(weights.data.permute(2, 0, 1).contiguous(),
        ...
        if self.iter_num % self.print_interval == 0:
            ...
            print("Iter {:05d}, Average Cross Entropy Loss: {:.4f}; Average MultiTask Loss: {:.4f}; "
                  "Average Training Loss: {:.4f}".format(self.iter_num,
            ...
            self.file_out.write("Iter {:05d}, Average Cross Entropy Loss: {:.4f}; "
                                "Average MultiTask Loss: {:.4f}; "
